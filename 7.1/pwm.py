...
#the threshold for the triangle wave frequency
freq_threshold=1000
#the tolerance for the frequency
tolerance=100
#the tolerance for the amplitude
amp_tolerance=0.05

#go through every possible combination and see which ones satisfy the requirements
for i in range(len(r1)):
    for j in range(len(r2)):
        for k in range(len(r3)):
            for l in range(len(c1)):
                actual_freq=(1/(4*r1[i]*c1[l]))*r3[k]/r2[j]
                if((actual_freq-freq_threshold)<=tolerance and actual_freq-freq_threshold>=0 and abs(r2[j]/r3[k]-0.2)<=amp_tolerance):
                    print(r1[i], r2[j], r3[k], c1[l], actual_freq, (r2[j]/r3[k])*5)
